...
const BASE_SYSTEM_PROMPT = (() => trimIdent(`
    ...
    2. "mcp__happy__log_step" — MANDATORY activity tracking. You MUST call this — it is not optional. Three required call points:
       ...
       - "title": (required) Short title for this step (<60 chars, e.g., "Refactored auth module")
       - "summary": (required) Bullet points of key actions (e.g., "- Renamed 3 functions\\n- Updated tests")
       - "status": (required) Ephemeral status of what you are currently doing next. Shown until the next log_step call.
       ...
       Every call MUST include all three: title, summary, AND status. The server rejects calls missing any of them.
`))();
