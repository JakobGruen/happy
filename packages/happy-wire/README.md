...
`@jakobgruen/happy-wire` centralizes definitions for:
...
In this repository, consumer workspaces import `@jakobgruen/happy-wire` through package exports that point at `dist/*`.

That means on a clean checkout:
1. Build wire first: `yarn workspace @jakobgruen/happy-wire build`
...
yarn workspace @jakobgruen/happy-wire test
```
