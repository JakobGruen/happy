...
export const es: TranslationStructure = {
    ...
    agentInput: {
        ...
        maxAttachments: 'Máximo 4 imágenes por mensaje',
        viewToggle: {
            chat: 'Chat',
            log: 'Log',
        },
    ...
    settingsVoice: {
        ...
        progressUpdatesTitle: 'Actualizaciones de progreso',
        progressUpdatesDescription: 'Cuando está activado, el asistente de voz resumirá periódicamente lo que el agente está haciendo mientras trabaja.',
        progressUpdatesEnabled: 'Actualizaciones automáticas',
        progressUpdatesEnabledSubtitle: 'Actualizaciones de voz durante sesiones activas',
        progressInterval: 'Intervalo de actualización',
        progressIntervalValue: ({ seconds }: { seconds: number }) => `${seconds}s`,

        // Voice personalization
        personalizationTitle: 'Personalización',
        personalizationDescription: 'Personaliza tu asistente de voz. Estos ajustes se aplican tanto a las sesiones de voz en vivo como a los mensajes de voz.',
        assistantName: 'Nombre del asistente',
        assistantNamePlaceholder: 'Happy',
        assistantNameSubtitle: 'Nombre usado en presentaciones y saludos',
        assistantBio: 'Sobre ti',
        assistantBioPlaceholder: 'Cuéntale al asistente sobre ti...',
        assistantBioSubtitle: 'Contexto personal para respuestas más relevantes',
        assistantSetup: 'Tu configuración de CC',
        assistantSetupPlaceholder: 'Describe tu configuración de Claude Code...',
        assistantSetupSubtitle: 'Herramientas, flujos de trabajo y casos de uso comunes',
    },
    ...
    notifications: {
        ...
        permissionQuestion: 'Tiene una pregunta para ti',
        permissionPlanReview: 'Plan listo para revisar',
        morePermissions: ({ count }: { count: number }) => `+${count} más pendientes`,
        browserAllow: 'Permitir',
        browserDeny: 'Denegar',
    },
    ...
    claude: {
        ...
        permissions: {
            ...
            sendFeedback: 'Enviar',
            feedbackPlaceholder: 'Dile a Claude por qué...',
        }
    },

    // Dynamic permission suggestion labels (from CC permission_suggestions)
    permissions: {
        forSession: 'esta sesión',
        forLocalSettings: 'configuración local del proyecto',
        forProject: 'este proyecto',
        forAllProjects: 'todos los proyectos',
        allowTool: ({ tool, scope }: { tool: string; scope: string }) => `Permitir ${tool} para ${scope}`,
        denyTool: ({ tool, scope }: { tool: string; scope: string }) => `Denegar ${tool} para ${scope}`,
        acceptAllEdits: ({ scope }: { scope: string }) => `Aceptar todas las ediciones para ${scope}`,
        bypassPermissions: ({ scope }: { scope: string }) => `Omitir todos los permisos para ${scope}`,
        planMode: ({ scope }: { scope: string }) => `Modo de planificación para ${scope}`,
        setMode: ({ mode, scope }: { mode: string; scope: string }) => `Establecer ${mode} para ${scope}`,
        addDirectories: ({ scope }: { scope: string }) => `Agregar directorios para ${scope}`,
