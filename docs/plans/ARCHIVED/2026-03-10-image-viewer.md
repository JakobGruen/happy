...
**Goal:** Add a fullscreen image viewer with pinch-to-zoom, pan, gallery swipe, and swipe-down-to-dismiss to the happy-app.

**Architecture:** Imperative singleton manager (`ImageViewerManager`) + React component (`ImageViewer`) mounted at root layout. Uses `react-native-reanimated` for animations and `react-native-gesture-handler` for gestures. No new dependencies.

**Tech Stack:** React Native, Expo Image, react-native-reanimated v4, react-native-gesture-handler v2, Unistyles

---

### Task 1: Create ImageViewer component with manager

**Files:**
- Create: `packages/happy-app/sources/components/ImageViewer.tsx`

**Step 1: Create the ImageViewer component file**

Create `packages/happy-app/sources/components/ImageViewer.tsx` with the following:
...
```bash
git add packages/happy-app/sources/components/ImageViewer.tsx
git commit -m "feat(app): add fullscreen image viewer component with pinch-to-zoom"
```

---

### Task 2: Mount ImageViewer in root layout
...
Inside the render, add `<ImageViewer />` after `<PermissionBanner />` (around line 238-239):
...
Place `<ImageViewer />` AFTER `</HorizontalSafeAreaWrapper>` but still inside `<GestureHandlerRootView>`. It must be outside the safe area wrapper so it covers the full screen.
...
git commit -m "feat(app): mount ImageViewer in root layout"
```

---

### Task 3: Wire thumbnails in MessageView to open viewer
...
Add at top of `MessageView.tsx`:
```typescript
import { ImageViewerManager } from './ImageViewer';
```

**Step 2: Wrap thumbnail images in Pressable**

Replace the image rendering block in `UserTextBlock` (lines 88-98):
...
Note: `Pressable` is already imported from `react-native` in this file.
...
```bash
git add packages/happy-app/sources/components/MessageView.tsx
git commit -m "feat(app): wire image thumbnails to fullscreen viewer"
```
...
Run: `yarn workspace happy-app web` (or native)

**Step 2: Test scenarios**

1. Send a message with 1 image → tap thumbnail → verify fullscreen with zoom
2. Send a message with multiple images → tap any thumbnail → verify gallery swipe
3. In fullscreen: pinch to zoom → pan while zoomed → double-tap to toggle zoom
4. In fullscreen: swipe down → verify dismissal
5. In fullscreen: tap X button → verify close
6. In fullscreen: single tap background (not zoomed) → verify close
7. Verify non-image messages still render correctly
