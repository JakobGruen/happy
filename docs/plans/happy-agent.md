...
- **Server API**: REST endpoints at `https://happy-server.green-wald.de` + Socket.IO at `/v1/updates`
...
- [x] Create `src/config.ts` — reads `HAPPY_SERVER_URL` (default: `https://happy-server.green-wald.de`), `HAPPY_HOME_DIR` (default: `~/.happy`), derives credential file path as `${happyHomeDir}/agent.key`
