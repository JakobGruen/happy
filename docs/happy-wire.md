...
This document describes the shared wire package: `@jakobgruen/happy-wire`.
...
`@jakobgruen/happy-wire` centralizes those shared schemas and types so all clients and services agree on the same wire contract.

## Package identity

- npm name: `@jakobgruen/happy-wire`
...
Shared from `@jakobgruen/happy-wire`:
...
- Session protocol imports now reference `@jakobgruen/happy-wire` directly.
- `src/sessionProtocol/types.ts` now re-exports from `@jakobgruen/happy-wire` as compatibility shim.
- API wire schemas in `src/api/types.ts` now source shared message/update schemas from `@jakobgruen/happy-wire`.

### App (`packages/happy-app`)

- Shared API message/update schemas in `sources/sync/apiTypes.ts` now import these from `@jakobgruen/happy-wire`:
  - `ApiMessageSchema`
...
- Prisma JSON message content type now references `SessionMessageContent` from `@jakobgruen/happy-wire`.
- Event router uses shared `SessionMessageContent` type for `new-message` payload typing.

### Agent (`packages/happy-agent`)

- `RawMessage` now aliases `SessionMessage` from `@jakobgruen/happy-wire`.

## Versioning model

All other workspace packages now declare a versioned dependency on `@jakobgruen/happy-wire`.

This intentionally mirrors post-publish consumption and reduces hidden coupling to workspace-local files.

## Build and release

`@jakobgruen/happy-wire` is configured the same way as existing publishable libraries in this repo:
...
```bash
yarn workspace @jakobgruen/happy-wire release
```

When building workspaces from a clean checkout, build `@jakobgruen/happy-wire` first so dependent packages can resolve generated `dist` outputs.
...
3. Bump and release `@jakobgruen/happy-wire`.
